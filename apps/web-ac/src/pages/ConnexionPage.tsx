    ...
    <div className="min-h-screen bg-gray-100 flex items-center justify-center px-4">
      <div className="max-w-md w-full">
        <div className="text-center mb-8">
          <h1 className="text-2xl font-bold text-[#1e3a5f]">eProcurement Benin</h1>
          <p className="text-sm text-gray-500 mt-1">Portail Autorite Contractante</p>
        </div>
        <form onSubmit={handleSubmit} className="bg-white rounded-xl shadow-sm border border-gray-200 p-8">
          ...
          <button
            type="submit"
            disabled={loading}
            className="w-full mt-6 bg-[#1e3a5f] text-white py-2.5 rounded-lg text-sm font-medium hover:bg-[#2a4d7a] transition-colors disabled:opacity-50"
          >
        ...
        <div className="mt-4 bg-amber-50 border border-amber-200 rounded-xl p-4">
          ...
          <div className="space-y-2">
            {[
              { email: 'prmp@anac.bj', password: '1234', label: 'PRMP ANAC', role: 'Séverin QUENUM' },
              { email: 'agent-ppm@anac.bj', password: 'EprocTest2026!', label: 'Agent PPM ANAC', role: 'Grâce DOSSOU-YOVO' },
              { email: 'evaluateur@anac.bj', password: 'EprocTest2026!', label: 'Evaluateur ANAC', role: 'Patrice BOKO' },
              { email: 'prmp@andf.bj', password: '1234', label: 'PRMP ANDF', role: 'Arnaud Hounkpatin' },
              { email: 'admin@eproc.bj', password: 'EprocTest2026!', label: 'Admin Systeme', role: 'Admin Systeme' },
            ].map((account) => (
              <button
                key={account.email}
                type="button"
